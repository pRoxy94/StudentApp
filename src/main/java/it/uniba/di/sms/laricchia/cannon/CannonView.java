...
public class CannonView extends SurfaceView implements
   ...
   private Linea blocker; // start and end points of the blocker
   ...
   private Paint cannonBPaint; // Paint used to draw the barrel --> add for 10ex
   ...
   public CannonView(Context context, AttributeSet attrs)
   {
      ...
      blocker = new Linea(); // create the blocker as a Linea
      ...
      cannonBPaint = new Paint();
      cannonBPaint.setColor(Color.WHITE);

      blockerPaint = new Paint(); // Paint for drawing the blocker
      blockerPaint.setColor(Color.WHITE);
   ...
   protected void onSizeChanged(int w, int h, int oldw, int oldh) {
      ...
      cannonPaint.setStrokeWidth(lineWidth * 0.5f); // set line thickness
      //Lo Stile specifica se la primitiva che viene disegnata è piena, tratteggiata o
      // entrambe (nello stesso colore).
      cannonPaint.setStyle(Paint.Style.STROKE);

      cannonBPaint.setStrokeWidth(w / 36);

      blockerPaint.setStrokeWidth(lineWidth * 0.4f); // set line thickness
      blockerPaint.setStyle(Paint.Style.STROKE);

      targetPaint.setStrokeWidth(lineWidth * 0.4f); // set line thickness
      targetPaint.setStyle(Paint.Style.STROKE);

      backgroundPaint.setColor(Color.BLACK); // set background color black
   ...
   public void drawGameElements(Canvas canvas) {
      ...
      // draw the cannon barrel --> set barrelEnd.x + 10
      canvas.drawLine(0, screenHeight / 2, barrelEnd.x + 10, barrelEnd.y, cannonPaint);

      // draw the cannon base
      canvas.drawCircle(0, (int) screenHeight / 2, (int) cannonBaseRadius, cannonPaint);

      // draw the blocker --> set drawRect instead of drawLine and set blocker.end.x
      canvas.drawRect(blocker.start.x, blocker.start.y, blocker.end.x - 35, blocker.end.y, blockerPaint);
      ...
      for (int i = 1; i <= TARGET_PIECES; ++i) {
         ...
         if (!hitStates[i - 1]) {
            // alternate coloring the pieces yellow and blue --> set all white for 10 ex
            // set drawRect instead of drawLine and target.end.x - 35
            canvas.drawRect(currentPoint.x, currentPoint.y, target.end.x - 35,
