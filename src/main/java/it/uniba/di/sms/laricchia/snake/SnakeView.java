...
package it.uniba.di.sms.laricchia.snake;
...
public class SnakeView extends TileView {
    ...
    private void updateSnake() {
        ...
        switch (mDirection) {
            case EAST: {
                newHead = new Coordinate(loopSnake(head.x + 1), head.y);
                break;
            }
            case WEST: {
                newHead = new Coordinate(loopSnake(head.x - 1), head.y);
        ...
        if (((newHead.x == 0 || newHead.x == mXTileCount-1 ) &&
                ( newHead.y > 1 && newHead.y < mYTileCount-2 )) ||
